...
<?php if ($f->status != 'PAGO'): ?><a href="<?= base_url(); ?>faturamento/baixar/<?= $f->idfatura; ?>" class="btn btn-sm btn-outline-success py-1 px-2" title="Baixar"><i class="fas fa-check"></i></a><?php endif; ?>
<button class="btn btn-sm btn-outline-primary py-1 px-2 btn-edit" data-id="<?= $f->idfatura; ?>"><i class="fas fa-edit"></i></button>
<a href="<?= base_url(); ?>faturamento/delete/<?= $f->idfatura; ?>" class="btn btn-sm btn-outline-danger py-1 px-2" onclick="return confirm('Excluir?');"><i class="fas fa-trash"></i></a>
